...
GAVEN_API void networking_recieve_init(networking_recieve *Event,http_connection* Connection,char* Recieved_Data,size_t Size);
...
GAVEN_API void networking_new_connection_init(networking_new_connection *Event,http_connection *Connection,char Ip[], uint16_t Port);

typedef struct http_response http_response;
typedef struct http_request http_request;
GAVEN_API http *create_http_server(const char* address,uint16_t port);
GAVEN_API http *create_http_client(void);
GAVEN_API http_connection *connect_http_client(http* client,char address[],uint16_t port);
