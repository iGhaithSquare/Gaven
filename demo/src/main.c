...
int example_on_networking_recieve(networking_recieve* E){
    ...
    size_t recieved_size =E->Size;
    char* Recieved_Data=E->Recieved_Data;
    size_t i;

    while(recieved_size>=512){
        GAVEN_INFO("Data recieved: %.*s",511,Recieved_Data);
        Recieved_Data+=511;
        recieved_size-=512;
    }GAVEN_INFO("Data recieved: Size %zu: %.*s",
           E->Size,
           (int)recieved_size,
           Recieved_Data);
    return 1;
...
int main(){
    ...
    send_http_request(connect_http_client(data->Client,"localhost",5000),HTTP_GET_REQUEST,"","/json","Host: localhost\r\n");
